  ...
  return (
    <div className="flex min-h-[calc(100vh-6rem)] items-center justify-center p-4">
      <div className="w-full max-w-md rounded-lg p-8 glass-panel">
        <h2 className="mb-2 text-3xl font-extrabold tracking-tight text-white">Create account</h2>
        <p className="mb-6 text-sm text-brand-muted">Provision a secure profile for receipt analysis.</p>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          <input {...register('name')} type="text" placeholder="Full name" className="w-full rounded-lg border border-white/10 bg-brand-dark px-4 py-3 text-white outline-none transition focus:border-brand-accent" required />
          ...
          <input {...register('password')} type="password" minLength={8} placeholder="Password" className="w-full rounded-lg border border-white/10 bg-brand-dark px-4 py-3 text-white outline-none transition focus:border-brand-accent" required />
